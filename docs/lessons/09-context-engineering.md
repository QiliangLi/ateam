...
F041 的 14 轮 review 看起来是"14 次独立验证"。但 0.95¹⁴ 的前提是**每次验证是独立的**。

它们不是。

14 轮 review 全部使用相同的上下文（spec + 代码 diff），全部缺少相同的信息（原始 Discussion）。这是**关联错误**——所有 reviewer 共享同一个盲区。关联错误比独立错误更危险：
