...
3. **分层进度**：小改动记 `progress.md`，大里程碑记 `claude-progress.txt`（详见 `docs/PROGRESS_FILES.md`）
...
> **重要说明**：这里的"任务"是指**开发编码时的工作任务**（如修复 bug、实现新功能、更新文档等），**不是**应用运行时用户发送给 agent 的消息。Agent 回复用户消息时不需要输出门禁声明。
...
**⚠️ 代码修改 checklist**（6 步）→ 改 .js/.ts/.tsx 前后执行 → `docs/workflows/checklist-code-change.md`
...
**硬规则：测试通过之后的下一步永远是输出门禁模板 → Code Review，不是"完成"或"进入 P1.x"。**
...
1. [ ] 测试绿后立即输出上面模板 → 2. [ ] Code Review（用 Task 启独立 agent 审查，不能自己审自己）产出 P0/P1/P2 → 3. [ ] **全量处置 CR 发现**：P0/P1 必须修复；P2 逐条列出处置方案（修复 / 不修+理由）呈现给用户确认，用户确认前不得跳过 → 修复后全量回归 → 二次 CR（同样用独立 agent，审查范围 = 本 phase 全部改动文件的完整复审，不是只看修复 diff）→ 归零声明（每条：修复了什么 → 怎么引入的 → 归因路径 A/B/C/D → checklist 是否需要更新，四步缺一不可）→ 4. [ ] ST 通过 → 5. [ ] 门禁结论"全部通过"后才能更新进度 → 6. [ ] **提交到 GitHub**
...
**硬规则：代码修改审核通过后必须提交到 GitHub**（DEV-54）。Phase 完成门禁"全部通过"后，下一步永远是 `git add + git commit + git push`，禁止跳过。commit message 格式：`[Mx-Py] 简短描述`（如 `[M1-P1] 实现猫猫召唤功能`）。小改动（单文件小修/Bug修复）CR 通过后同样必须提交。
...
- **LLM 输出防御**（DEV-BUG-16）：接收 LLM 输出的数据模型必须：① 枚举/有限值字段加 validator 做 normalize + 拒绝非法值 ② 数值字段加 coerce ③ 字符串字段加 strip。**LLM 输出永远不可信，数据校验是最后防线**
...
| 类别 | 路径 |
|------|------|
| 后端核心 | `server/`（servers/, a2a/, cli/, mcp/ 等） |
| 主入口 | `server/index.js` |
| 前端 | `web/` |
...
| 文件 | 说明 |
|------|------|
| `server/index.js` | 主入口，启动聊天/回调服务器 |
| `server/servers/chat-server.js` | WebSocket 聊天服务器 |
| `server/servers/callback-server.js` | MCP 回调服务 |
| `server/a2a/router.js` | Agent-to-Agent 路由 |
| `server/a2a/mentions.js` | @ 提及解析 |
| `server/a2a/registry.js` | 工作队列管理 |
| `server/cli/ai-cli.js` | AI CLI 核心封装，对接 Claude/Codex/Gemini |
| `server/mcp/server.js` | MCP 服务入口 |
| `server/config/cats.js` | 猫猫配置（布偶/缅因/暹罗） |
...
| 路径 | 状态 | 说明 |
|------|------|------|
| `claude-progress.txt` | ✅ 已创建 | 进度记录文件 |
| `server/` | ✅ 已创建 | 后端代码目录 |
| `docs/ROADMAP.md` | ✅ 已创建 | 功能路线图 |
| `docs/VISION.md` | ✅ 已存在 | 项目愿景 |
| `docs/lessons/` | ✅ 已存在 | 教程文档 |
| `docs/runbooks/` | ✅ 已创建 | 工作手册（含错题本） |
| `docs/workflows/` | ✅ 已存在 | 工作流 checklist |
| `docs/personas/` | ✅ 已存在 | 五方评审角色定义 |
| `docs/api-contract.md` | ✅ 已创建 | 接口契约 |
| `docs/CODE_MAP.md` | ✅ 已创建 | 代码导航 |
| `docs/discussions.md` | ✅ 已创建 | 讨论索引 |
| `docs/PROGRESS_FILES.md` | ✅ 已创建 | 进度文件说明 |
